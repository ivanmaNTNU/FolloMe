...
public:
	...
	void makePath(array<array<Node, NUM_Y_TILES>, NUM_X_TILES> allMap, Node dest) {
		try {
			...
			while (!path.empty()) {
				...
				map.setValueSafe(top.x -map.xPos, top.y - map.yPos, PATH);
				//cout << "Path Value set" << endl;
			}
			map.setValue(destination.x - map.xPos, destination.y - map.yPos, DESTINATION);
			map.setValueSafe(destination.x - map.xPos, destination.y - map.yPos, DESTINATION);
		}
	...
	void findPath(){ 
		...
		if (!isValid(map.xPos - 1, map.yPos - 1) && !isValid(map.xPos - 1, map.yPos) && !isValid(map.xPos - 1, map.yPos + 1) &&
			!isValid(map.xPos + 0, map.yPos - 1) &&										!isValid(map.xPos + 0, map.yPos + 1) &&
			!isValid(map.xPos + 1, map.yPos - 1) && !isValid(map.xPos + 1, map.yPos) && !isValid(map.xPos + 1, map.yPos + 1)) {
			cout << "Robot is stuck at current position!" << endl;
			return;
		}
		if (!isValid(map.xPos, map.yPos)){
		...
		while (!openList.empty() && openList.size()<(NUM_X_TILES)*(NUM_Y_TILES)) {
			...
			if (count >= NUM_X_TILES*NUM_Y_TILES){ //FAIL SAFE: This is so that is something wrong happens, the program does not go into a infinite loop and crash. 
				...
				destination.x = map.xPos;
				destination.y = map.yPos;
				break;
			} 
		}
